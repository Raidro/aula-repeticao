let numero1 = parseFloat(prompt('Digite N1: '));
let numero2 = parseFloat(prompt('Digite N2: '));
let numero3 = parseFloat(prompt('Digite N3: '));
let numero4 = parseFloat(prompt('Digite N4: '));
let numero5 = parseFloat(prompt('Digite N5: '));
let soma = 0;
let madia = 0;

soma = numero1 + numero2 + numero3 + numero4 + numero5;
media = (numero1 + numero2 + numero3 + numero4 + numero5) / 5;

while (soma < 0 && media < 0) {
    alert('Valor Invalido!!');
    numero1 = parseFloat(prompt('Digite N1: '));
    numero2 = parseFloat(prompt('Digite N2: '));
    numero3 = parseFloat(prompt('Digite N3: '));
    numero4 = parseFloat(prompt('Digite N4: '));
    numero5 = parseFloat(prompt('Digite N5: '));

    soma = numero1 + numero2 + numero3 + numero4 + numero5;
    media = (numero1 + numero2 + numero3 + numero4 + numero5) / 5;
}



alert('A soma é: ' + soma + '\n' + 'A media é: ' + media)